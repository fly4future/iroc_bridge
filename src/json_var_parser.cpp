...
namespace iroc_bridge
{
  ...
  bool parse_var(const json& js, std::pair<std::string_view, parseable_t>& var)
  {
    ...
    {
      ROS_ERROR_STREAM_THROTTLE(1.0, "[IROCBridge]: JSON doesn't have the expected member \"" << var_name << "\".");
      return false;
    }

    auto& var_out = var.second;
    std::visit([var_name, &js](auto&& var_out)
        {
          ...
          {
            json type_check = T{};
            ROS_ERROR_STREAM_THROTTLE(1.0, "[IROCBridge]: Cannot parse member \"" << var_name << "\" (value: " << js.at(var_name) << ") as type \"" << type_check.type_name() << "\": " << e.what());
          }
        }, var_out);
    return true;
  }
